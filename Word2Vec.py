...
def cosine_similarity(embedding, valid_size=16, valid_window=100, device='cpu'):
    ...
    valid_examples = np.array(np.random.choice(range(valid_window), valid_size // 2))
    valid_examples = np.append(valid_examples, np.random.choice(range(1000, 1000 + valid_window), valid_size // 2))
...
def train(model, train_words, int2word, device, epochs=5, learning_rate=0.003, print_every=500):
    ...
    :param train_words: training text
    :param int2word: integer-word encoding dictionary
    :param epochs: no. of epochs to run
    :param learning_rate: learning rate for neural network
    :param print_every: steps interval for printing results
    ...
    for e in range(epochs):
        ...
        for inputs, targets in get_batches(train_words, 512):
            ...
            if steps % print_every == 0:
                ...
                print('...')
    print('\nMODEL TRAINING COMPLETED AT END OF EPOCH {}'.format(e + 1))

    return model


#%%
def tsne_viz(model, int2word, words_to_chart=600):
    """
    Implements the visualization of a selected number of words from the corpus with tSNE.
    :param model: trained SkipGram model
    :param int2word: integer-word encoding dictionary
    :param words_to_chart: no. of words to chart
    :return: tSNE chart of selected number of words
    """
    ...
    plt.savefig('skipgram_tSNE_{}'.format(datetime.today().date(), int(datetime.today().timestamp())))
...
def main():
    ...
    print('\nTotal number of words in text: {}'.format(len(words)))
    ...
    print('\nTraining on {}'.format('CUDA' if device == 'cuda' else 'CPU'))
